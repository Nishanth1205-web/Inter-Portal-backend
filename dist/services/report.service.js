...
class ReportService {
    async getStudentReport(params) {
        ...
        const [students, total] = await Promise.all([
            config_1.prisma.student.findMany({
                where,
                skip,
                take: limit,
                include: {
            ...
            config_1.prisma.student.count({ where }),
        ]);
    ...
    async exportExcel(data, headers, sheetName = 'Report') {
    ...
    async exportPDF(data, headers, title = 'Report') {
